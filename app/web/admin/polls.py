...
async def close_poll(require_admin, poll_db_id, telegram_app, telegram_chat_id, send_to_group, announce_winner, logger):
    ...
    try:
        ...
        if poll.get("poll_type") == "books" and telegram_chat_id:
            ...
            if len(tied) > 1:
                tie_lines = [
                    "⚖️ Hay empate en la votación",
                    "",
                    f"Estos libros han terminado con {tied[0]['votes']} votos:",
                ]
                for book in tied:
                    label = book["title"]
                    if book.get("author"):
                        label += f" — {book['author']}"
                    tie_lines.append(f"• {label}")
                tie_lines.extend(["", "🔁 Lanzo ahora una encuesta de desempate."])
                await send_to_group("\n".join(tie_lines), parse_mode=None, message_type="tie_notification")
...
async def create_dates_poll(require_admin, meeting_id, telegram_app, telegram_chat_id, logger):
    ...
    try:
        ...
        if len(date_options) < 2:
            return "Anade al menos 2 opciones de fecha primero", 400
        if not telegram_chat_id:
            return "TELEGRAM_CHAT_ID no configurado", 500
            ...
            for option in date_opts:
                ...
                if winner_text[:16] in opt_str[:20] or opt_str[:16] in winner_text[:20]:
                    ...
                    await send_to_group(
                        "\n".join(
                            [
                                "📅 Fecha cerrada para la reunion",
                                "",
                                f"📚 {meeting_name}",
                                f"🕒 {opt_str}",
                                "",
                                "✅ Ya puedes confirmar asistencia con /asistir",
                            ]
                        ),
                        parse_mode=None,
