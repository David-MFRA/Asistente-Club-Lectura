...
async def wizard_new_cycle(require_admin, send_to_group, utcnow, logger):
    ...
    try:
        text = (
            ...
            "Ha llegado el momento de elegir nuestro proximo libro.\n\n"
            "Sugiere tus propuestas con el comando:\n"
...
async def wizard_lock_and_poll(require_admin, telegram_app, telegram_chat_id, logger):
    ...
    try:
        ...
        msg = await telegram_app.bot.send_poll(
            ...
            question="¿Que libro leemos este ciclo?",
            options=options,
            is_anonymous=False,
            allows_multiple_answers=False,
        )
        db.save_poll(chat_id=msg.chat_id, message_id=msg.message_id, poll_id=msg.poll.id, poll_type="books", cycle_key=cycle)
